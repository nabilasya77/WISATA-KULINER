...
$mode = 'tambah';
$data = [
    'id' => '', 'nama_destinasi' => '', 'kategori_id' => '', 'deskripsi' => '',
    'tiket_masuk' => 0, 'lokasi' => '', 'alamat_lengkap' => '', 'latitude' => '',
    'longitude' => '', 'jam_operasional' => '', 'kontak' => '', 'gambar_utama' => '',
    'fasilitas' => '', 'is_unggulan' => 0, 'status' => 'aktif'
];

// Mode Edit: ambil data lama jika ada parameter id
if (isset($_GET['id'])) {
    $mode = 'edit';
    $id = (int) $_GET['id'];
    $cek = mysqli_query($koneksi, "SELECT * FROM destinasi_wisata WHERE id = $id");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $tiket_masuk     = (float) $_POST['tiket_masuk'];
    ...
    $status          = $_POST['status'];
    $slug            = buatSlug($nama_destinasi) . '-' . time();
    ...
    if ($nama_destinasi === '') {
        ...
        if ($error === '') {
            if ($mode === 'edit' && $id_post > 0) {
                $stmt = mysqli_prepare($koneksi, "UPDATE destinasi_wisata SET nama_destinasi=?, slug=?, kategori_id=?, deskripsi=?, tiket_masuk=?, lokasi=?, alamat_lengkap=?, latitude=?, longitude=?, jam_operasional=?, kontak=?, gambar_utama=?, fasilitas=?, is_unggulan=?, status=? WHERE id=?");
                mysqli_stmt_bind_param($stmt, "ssisdsssddsssisi",
                    $nama_destinasi, $slug, $kategori_id, $deskripsi, $tiket_masuk, $lokasi,
                    $alamat_lengkap, $latitude, $longitude, $jam_operasional, $kontak,
                    $gambar_utama, $fasilitas, $is_unggulan, $status, $id_post
                );
                mysqli_stmt_execute($stmt);
                header("Location: destinasi.php?status=edit");
                exit;
            } else {
                $stmt = mysqli_prepare($koneksi, "INSERT INTO destinasi_wisata (nama_destinasi, slug, kategori_id, deskripsi, tiket_masuk, lokasi, alamat_lengkap, latitude, longitude, jam_operasional, kontak, gambar_utama, fasilitas, is_unggulan, status) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
                mysqli_stmt_bind_param($stmt, "ssisdsssddsssis",
                    $nama_destinasi, $slug, $kategori_id, $deskripsi, $tiket_masuk, $lokasi,
                    $alamat_lengkap, $latitude, $longitude, $jam_operasional, $kontak,
                    $gambar_utama, $fasilitas, $is_unggulan, $status
                );
                mysqli_stmt_execute($stmt);
                header("Location: destinasi.php?status=tambah");
                exit;
            }
        }
    }

    // Jika error, tampilkan kembali data yang diinput
    $data = array_merge($data, $_POST);
    $data['is_unggulan'] = $is_unggulan;
}

$q_kategori = mysqli_query($koneksi, "SELECT * FROM kategori_wisata ORDER BY nama_kategori ASC");
?>
<!DOCTYPE html>
<html lang="id">
<head>
    ...
    <title><?= $mode === 'edit' ? 'Edit' : 'Tambah' ?> Destinasi - Admin Wonogiri Wisata</title>
...
<body>
    ...
    <main class="main-content">
        ...
        <div class="container">
            <div class="breadcrumb"><a href="destinasi.php">Destinasi Wisata</a> / <?= $mode === 'edit' ? 'Edit' : 'Tambah' ?></div>
            <h1 class="page-title"><?= $mode === 'edit' ? 'Edit Destinasi' : 'Tambah Destinasi Baru' ?></h1>
            <br>
                ...
                <form method="POST">
                    ...
                    <div class="form-grid">
                        <div class="form-group full">
                            ...
                            <input type="text" name="nama_destinasi" required value="<?= htmlspecialchars($data['nama_destinasi']) ?>">
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <select name="kategori_id">
                                ...
                                <option value="<?= $kat['id'] ?>" <?= ($data['kategori_id'] == $kat['id']) ? 'selected' : '' ?>><?= htmlspecialchars($kat['nama_kategori']) ?></option>
                                <?php endwhile; ?>
                            </select>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" step="0.01" name="tiket_masuk" value="<?= htmlspecialchars($data['tiket_masuk']) ?>">
                        </div>

                        <div class="form-group">
                            <label>Lokasi (Kecamatan/Area)</label>
                        ...
                        <div class="form-group full">
                            ...
                            <input type="text" name="jam_operasional" value="<?= htmlspecialchars($data['jam_operasional']) ?>" placeholder="08.00 - 17.00 WIB">
                        </div>
                        ...
                        <div class="form-group full">
                            ...
                            <textarea name="fasilitas" placeholder="Toilet, Mushola, Parkir, Gazebo"><?= htmlspecialchars($data['fasilitas']) ?></textarea>
                        </div>
    ...
    <?php if (!empty($data['gambar_utama'])): ?>
        <img src="<?= htmlspecialchars($data['gambar_utama']) ?>" class="img-preview" style="max-width: 200px; display: block; margin-bottom: 10px;" onerror="this.style.display='none'">
    <?php endif; ?>
    
    <input type="text" name="gambar_utama" value="<?= htmlspecialchars($data['gambar_utama']) ?>" class="form-control" placeholder="Masukkan atau paste URL gambar dari internet (contoh: https://website.com/foto.jpg)">
</div>
                        <div class="form-group">
                            <label>Status</label>
                            <select name="status">
                        ...
                        <div class="form-group">
                            ...
                            <div class="checkbox-row">
                                <input type="checkbox" name="is_unggulan" id="unggulan" <?= !empty($data['is_unggulan']) ? 'checked' : '' ?>>
                                <label for="unggulan" style="margin:0;">Tandai sebagai Destinasi Unggulan</label>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="form-actions">
                        ...
                        <a href="destinasi.php" class="btn-cancel">Batal</a>
                    </div>
                </form>
            </div>
        </div>
