...
if (isset($_GET['hapus'])) {
    $id = (int) $_GET['hapus'];
    $cek = mysqli_query($koneksi, "SELECT gambar_utama FROM kuliner WHERE id = $id");
    ...
    header("Location: kuliner.php?status=hapus");
    exit;
}
...
if ($keyword !== '') {
    ...
    $where = "WHERE nama_kuliner LIKE '%$k%' OR lokasi LIKE '%$k%'";
}

$q_kuliner = mysqli_query($koneksi, "SELECT * FROM kuliner $where ORDER BY id DESC");
?>
<!DOCTYPE html>
<html lang="id">
<head>
    ...
    <title>Kuliner Lokal - Admin Wonogiri Wisata</title>
    ...
    <main class="main-content">
        <header class="topbar">
            <div class="topbar-search">
                <form method="GET" action="kuliner.php">
                    <input type="text" name="cari" placeholder="Cari menu / resto..." value="<?= htmlspecialchars($keyword) ?>">
        ...
        <div class="container">
            <div class="page-header">
                <h1 class="page-title">Kuliner Lokal</h1>
                <a href="kuliner_tambah.php" class="btn-add"><i class="fa-solid fa-plus"></i> Tambah Menu Kuliner</a>
            </div>

            <?php if (isset($_GET['status']) && $_GET['status'] === 'hapus'): ?>
                <div class="alert alert-success"><i class="fa-solid fa-circle-check"></i> Menu kuliner berhasil dihapus.</div>
            <?php elseif (isset($_GET['status']) && $_GET['status'] === 'tambah'): ?>
                <div class="alert alert-success"><i class="fa-solid fa-circle-check"></i> Menu kuliner baru berhasil ditambahkan.</div>
            <?php elseif (isset($_GET['status']) && $_GET['status'] === 'edit'): ?>
                <div class="alert alert-success"><i class="fa-solid fa-circle-check"></i> Menu kuliner berhasil diperbarui.</div>
            <?php endif; ?>
            ...
            <div class="card-table">
                <table>
                    ...
                    <tbody>
                        <?php $no = 1; while ($row = mysqli_fetch_assoc($q_kuliner)): ?>
                        <tr>
                            <td><?= $no++ ?></td>
                            ...
                            <td><?= formatRupiah($row['harga_mulai']) ?></td>
                            <td><i class="fa-solid fa-star" style="color: var(--accent);"></i> <?= $row['rating'] ?></td>
                            <td>
                            ...
                            <td class="action-btns">
                                <a href="kuliner_tambah.php?id=<?= $row['id'] ?>" class="btn-edit" title="Edit"><i class="fa-solid fa-pen"></i></a>
                                <a href="kuliner.php?hapus=<?= $row['id'] ?>" class="btn-delete" title="Hapus" onclick="return confirm('Yakin hapus menu kuliner ini?');"><i class="fa-solid fa-trash"></i></a>
                            </td>
                        </tr>
                        <?php endwhile; ?>
                        <?php if (mysqli_num_rows($q_kuliner) == 0): ?>
                        <tr><td colspan="8"><div class="empty-state"><i class="fa-solid fa-bowl-food fa-2x" style="margin-bottom:10px;"></i><br>Belum ada data kuliner.</div></td></tr>
                        <?php endif; ?>
                    </tbody>
                </table>
            </div>
